...
public class InfusionTableBlockEntity extends ActiveTile<InfusionTableBlockEntity> {
    ...
    public static void runLogicTickCycle(InfusionTableBlockEntity be) {
        ...
        if (be.shouldBeWorking && !be.isWorking) be.getInfusionRecipe(be.getInfusable().getStackInSlot(0), stacks);
        if (be.recipe != null && (be.shouldBeWorking || be.isWorking)) {
            ...
            if (!recipe.isValid(be.getInfusable().getStackInSlot(0), be.getPedestalStacks())) {
    ...
    public static void runAnimationTickCycle(BlockPos pos, InfusionTableBlockEntity blockEntity) {
        ...
        Player player = blockEntity.level.getNearestPlayer((double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D, 3.0D, false);
        if (player != null) {
