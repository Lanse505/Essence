    ...
    public List<ITextComponent> getRenderedText(Pair<Integer, CompoundNBT> info) {
        ...
        if (InputMappings.isKeyDown(handle, GLFW.GLFW_KEY_LEFT_SHIFT) || InputMappings.isKeyDown(handle, GLFW.GLFW_KEY_RIGHT_SHIFT)) {
            ...
            for (EffectInstance instance : effects) {
                ...
                textComponents.add(new TranslationTextComponent("brewed.duration", EssenceUtilHelper.getDurationString(instance.getDuration() / 20)).applyTextStyle(TextFormatting.LIGHT_PURPLE));
                textComponents.add(new TranslationTextComponent("brewed.amplifier", instance.getAmplifier()).applyTextStyle(TextFormatting.LIGHT_PURPLE));
            }
        } else {
