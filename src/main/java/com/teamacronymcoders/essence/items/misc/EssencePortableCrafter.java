...
public class EssencePortableCrafter extends Item implements IScreenAddonProvider, INamedContainerProvider {

    private final InventoryComponent<?> grid;
    private final InventoryComponent<?> output;

    public EssencePortableCrafter() {
        super(new Item.Properties().maxStackSize(1).group(Essence.TOOL_TAB).rarity(Rarity.RARE));
        grid = new InventoryComponent<>("grid", 0, 0, 9).setRange(3, 3);
        output = new InventoryComponent<>("grid", 0, 0, 1);
    }
